...
import '../database/counter/batch_view_counter_service.dart';
...
// 辅助方法：比较两个游戏列表是否相同
  ...
  Future<List<Game>> getGamesPaginated({
    ...
    String? authorId, // 新增authorId参数
  }) async {
    try {
      final cacheKey = 'paginated_games_${page}_${pageSize}_${sortBy}_${descending}_${authorId ?? "all"}';
      ...
      // 如果提供了authorId，添加作者过滤条件
      if (authorId != null) {
        query = query.eq('authorId', authorId);
      }

      final cursor = _dbConnectionService.games.find(query);
