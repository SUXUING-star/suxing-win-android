...
class CheckInContent extends StatelessWidget {
  ...
  static const Duration _delayIncrement = Duration(milliseconds: 100); // Stagger for vertical
  static const Duration _horizontalStagger = Duration(milliseconds: 150); // Stagger for horizontal right panel
  static const Duration _horizontalInternalStagger = Duration(milliseconds: 100); // Stagger within horizontal right panel
  ...
  Widget _buildStatsSummarySection(BuildContext context, Duration delay, Key key) {
  ...
  Widget _buildTodayCheckInSection(Duration delay, Key key, {double? maxHeight}) {
    return FadeInSlideUpItem(
      key: key,
      duration: _slideDuration,
      delay: delay,
  ...
  Widget _buildCalendarSection(Duration delay, Key key) {
    return FadeInSlideUpItem(
      key: key,
      duration: _slideDuration, // Or a different animation type/duration if preferred
      delay: delay,
      slideOffset: _slideOffset,
      child: CalendarView(
  ...
  Widget build(BuildContext context) {
    ...
    final rootKey = ValueKey('check_in_content_root_${currentUser.id}_${selectedYear}_$selectedMonth');
  ...
  Widget _buildHorizontalLayout(BuildContext context) {
    ...
    final calendarKey = ValueKey('calendar_horiz_${selectedYear}_$selectedMonth');
    final levelProgressKey = ValueKey('level_horiz_${currentUser.id}');
    final statsSummaryKey = ValueKey('stats_summary_horiz_${checkInStats.hashCode}');
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            flex: 3,
            child: SingleChildScrollView( // Keep SingleChildScrollView if calendar content can exceed flexbox space
                ...
                child: Column(
                  children: [
                    _buildLevelProgressSection(
                      _baseDelay + _horizontalStagger + (_horizontalInternalStagger * rightPanelDelayIndex++),
                      levelProgressKey,
                    ),
  ...
  Widget _buildVerticalLayout(BuildContext context) {
    ...
    final statsSummaryKey = ValueKey('stats_summary_vert_${checkInStats.hashCode}');
    final todayListKey = ValueKey('today_list_vert_${currentUser.id}');
    final calendarKey = ValueKey('calendar_vert_${selectedYear}_$selectedMonth');
