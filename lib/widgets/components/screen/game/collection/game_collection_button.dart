...
class _GameCollectionButtonState extends State<GameCollectionButton> {
  ...
  void didUpdateWidget(GameCollectionButton oldWidget) {
    ...
    print('GameCollectionButton (${widget.game.id}) didUpdateWidget. isLoading: $_isLoading. New initial from parent: ${widget.initialCollectionStatus?.status}');
    // 如果游戏 ID 变了，强制同步
    if (widget.game.id != oldWidget.game.id && !_isLoading) {
      print('GameCollectionButton (${widget.game.id}) Game ID changed, syncing local status.');
      setState(() { _collectionStatus = widget.initialCollectionStatus; });
      return;
    }
    // 如果父组件传来的状态变了，并且按钮不在加载中，且内容确实不同，才同步
    if (widget.initialCollectionStatus != oldWidget.initialCollectionStatus && !_isLoading) {
      bool contentChanged = _collectionStatus?.status != widget.initialCollectionStatus?.status ||
      ...
      if (contentChanged) {
        print('GameCollectionButton (${widget.game.id}) Parent initialStatus differs from local and not loading, syncing local status.');
        setState(() { _collectionStatus = widget.initialCollectionStatus; });
      } else {
        print('GameCollectionButton (${widget.game.id}) Parent initialStatus same as local or button loading, skipping sync.');
      }
    }
  }
  ...
  Future<void> _showCollectionDialog() async {
    ...
    final result = await showGeneralDialog<Map<String, dynamic>>(
      ...
      transitionDuration: const Duration(milliseconds: 350), // 动画时长，和 CustomConfirmDialog 一致
    ...
    if (result != null && mounted) {
      ...
      Map<String, int> countDeltas = {'want': 0, 'playing': 0, 'played': 0, 'total': 0}; // 默认增量为0
      ...
      try {
        if (action == 'set') {
          ...
          // 调用 Service
          final (item, returnedStatus) = await _collectionService.setGameCollection(
          ...
          if (item != null && returnedStatus == status) {
            ...
            // *** 计算增量 ***
            countDeltas = _calculateCountDeltas(oldStatus?.status, finalNewStatus?.status);
            print('GameCollectionButton (${widget.game.id}): set SUCCESS. New status recorded: ${finalNewStatus?.status}. Deltas: $countDeltas');
            if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('收藏状态已更新')));
          } else {
            print('GameCollectionButton (${widget.game.id}): set FAILED. Service response invalid.');
            throw Exception("更新收藏失败");
          }

        } else if (action == 'remove') {
          // 调用 Service
          final success = await _collectionService.removeGameCollection(widget.game.id);
          if (success) {
            finalNewStatus = null; // API 调用成功，新状态为 null
            // *** 计算增量 ***
            countDeltas = _calculateCountDeltas(oldStatus?.status, null);
            print('GameCollectionButton (${widget.game.id}): remove SUCCESS. New status recorded as null. Deltas: $countDeltas');
            if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('已从收藏中移除')));
          } else {
            print('GameCollectionButton (${widget.game.id}): remove FAILED.');
        ...
        if (mounted) {
          ...
          setState(() {
            ...
            print('GameCollectionButton (${widget.game.id}): setState internal _collectionStatus to ${finalNewStatus?.status}');
          });

          // 2. 调用回调，传递包含新状态和增量的结果对象给父组件
          final changeResult = CollectionChangeResult(newStatus: finalNewStatus, countDeltas: countDeltas);
          print('GameCollectionButton (${widget.game.id}): Calling onCollectionChanged with result: $changeResult');
          widget.onCollectionChanged?.call(changeResult); // <--- 传递结果对象
        }

      } catch (e) {
        // 统一处理 Service 调用失败或解析失败的异常
        print('GameCollectionButton (${widget.game.id}): Operation error in dialog handler: $e');
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('操作失败: ${e.toString().split(':').last.trim()}')));
          // 失败时，本地 _collectionStatus 保持不变 (即 API 操作前的状态)
          // *** 失败时不调用父组件的回调 ***
        }
      } finally {
        // *** 无论成功失败，结束加载状态 ***
        if (mounted) {
          ...
          print('GameCollectionButton (${widget.game.id}): finally - Setting isLoading=false.');
        }
      }
    } else {
      // 用户取消了对话框
      print('GameCollectionButton (${widget.game.id}): Collection dialog cancelled.');
    }
  }

  /// 计算收藏状态变化导致的计数增量
  Map<String, int> _calculateCountDeltas(String? oldStatusString, String? newStatusString) {
    Map<String, int> deltas = {'want': 0, 'playing': 0, 'played': 0, 'total': 0};
    ...
    // 1. 处理旧状态的减少
    if (hadStatus) {
      if (oldStatusString == GameCollectionStatus.wantToPlay) deltas['want'] = -1;
      if (oldStatusString == GameCollectionStatus.playing) deltas['playing'] = -1;
      if (oldStatusString == GameCollectionStatus.played) deltas['played'] = -1;
    }

    // 2. 处理新状态的增加
    if (hasStatus) {
      if (newStatusString == GameCollectionStatus.wantToPlay) deltas['want'] = (deltas['want'] ?? 0) + 1;
      if (newStatusString == GameCollectionStatus.playing) deltas['playing'] = (deltas['playing'] ?? 0) + 1;
      if (newStatusString == GameCollectionStatus.played) deltas['played'] = (deltas['played'] ?? 0) + 1;
    }

    // 3. 处理总数变化
    if (hadStatus && !hasStatus) { // 从有状态到无状态（移除）
      deltas['total'] = -1;
    } else if (!hadStatus && hasStatus) { // 从无状态到有状态（新增）
      deltas['total'] = 1;
    } else { // 状态变化 (want -> playing) 或无变化
      deltas['total'] = 0;
    }
  ...
  Widget _buildAddCollectionButton(ThemeData theme) {
    if (widget.compact) {
      return IconButton(
        icon: _isLoading
            ? SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: theme.primaryColor))
  ...
  Widget _buildCollectionStatusButton(String status, ThemeData theme) {
    ...
    if (widget.compact) {
      ...
      return OutlinedButton.icon(
        icon: _isLoading
            ? SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, valueColor: AlwaysStoppedAnimation(textColor)))
