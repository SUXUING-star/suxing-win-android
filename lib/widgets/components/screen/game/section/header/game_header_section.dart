// lib/widgets/components/screen/game/section/header/game_header_section.dart

/// 该文件定义了 [GameHeaderSection] 组件，用于显示游戏详情页的头部信息。
/// [GameHeaderSection] 负责展示游戏标题、简介、标签、分类和用户互动状态。
library;
...
import 'package:suxingchahui/widgets/components/screen/game/section/tag/game_tags_section.dart'; // 游戏标签组件
...
/// [GameHeaderSection] 类：游戏详情页的头部组件。
///
...
class GameHeaderSection extends StatelessWidget {
  ...
  final FutureVoidCallback? onToggleLike; // 切换点赞状态的回调
  final FutureVoidCallback? onToggleCoin; // 切换投币状态的回调
  final FutureVoidCallback? onShare; // 切换投币状态的回调
      ...
      child: Row(
        ...
        children: [
          _buildInteractionButton(
            isLoading: isSharing,
            isDone: hasShared,
            doneIcon: Icons.share_sharp,
            undoneIcon: Icons.share_outlined,
            doneColor: Colors.lightBlueAccent.shade400,
            undoneColor: greyColor,
            onPressed: onShare,
            tooltip: hasShared ? '再次分享' : '分享',
          ),
          ...
          const SizedBox(width: 4), // 间距
          Text(
            rating != null ? rating.toString() : game.rating.toString(), // 评分文本
            style: TextStyle(
              fontSize: 14,
              fontWeight: FontWeight.bold,
  ...
  Widget _buildInteractionButton({
    ...
    required FutureVoidCallback? onPressed,
        ...
        Wrap(
          ...
          children: [
            ...
            _buildMetaItem(
                ...
                likeCount != null ? '$likeCount 人点赞' : '${game.likeCount} 人点赞',
                textStyle),
            _buildMetaItem(
            ...
            _buildMetaItem(
                ...
                rating != null ? '评分值 $rating' : '评分值 ${game.rating}',
                textStyle), // 评分
          ],
        ),
        const SizedBox(height: 8), // 间距
        _buildPublishTime(textStyle),
      ],
    );
  }
  ...
  ///
  ///
  Widget _buildPublishTime(TextStyle textStyle) {
    return Row(
      children: [
        Icon(Icons.access_time, size: 16, color: Colors.grey[600]), // 时间图标
        const SizedBox(width: 4), // 间距
        Text('发布于 ${game.createTime.formatTimeAgo()}', // 发布时间
            style: textStyle),
        const SizedBox(width: 8), // 间距
        Text('·', style: textStyle), // 分隔符
        const SizedBox(width: 8), // 间距
        Text('更新于 ${game.updateTime.formatTimeAgo()}', // 更新时间
            style: textStyle),
      ],
    );
  }
