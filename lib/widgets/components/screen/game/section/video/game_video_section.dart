// lib/widgets/components/screen/game/section/video/game_video_section.dart
...
class GameVideoSection extends StatefulWidget {
  final String? bilibiliVideoUrl; // 视频的 Bilibili bvid
  ...
  const GameVideoSection({
    super.key,
    required this.bilibiliVideoUrl,
  });
...
class _GameVideoSectionState extends State<GameVideoSection> {
  ...
  void didUpdateWidget(covariant GameVideoSection oldWidget) {
    ...
    if (widget.bilibiliVideoUrl != oldWidget.bilibiliVideoUrl) {
      // 检查 bvid 是否发生变化
      if (mounted) {
        // 检查组件是否已挂载
        setState(() {
  ...
  void _startLoadingVideo() {
    if (widget.bilibiliVideoUrl != null && mounted) {
      // 检查 URL 有效且组件已挂载
      setState(() {
  ...
  Widget build(BuildContext context) {
    final currentEmbedUrl = widget.bilibiliVideoUrl; // 获取当前嵌入 URL
  ...
  Widget _buildContent(String currentEmbedUrl) {
    if (_loadingError != null) {
      ...
      // 显示 WebView 和加载指示器
      return Stack(
        key: ValueKey('video_stack_${widget.bilibiliVideoUrl}'), // Key
