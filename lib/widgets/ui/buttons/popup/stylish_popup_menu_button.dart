...
class StylishPopupMenuButton<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final ShapeBorder effectiveShape = shape ?? RoundedRectangleBorder( // 决定菜单形状
      borderRadius: BorderRadius.circular(8.0),
    );

    // 决定触发器 Widget
    final Widget trigger = child ?? Icon(
      icon,
      size: iconSize,
      color: isEnabled
      ...
      itemBuilder: (BuildContext context) {
        ...
        for (int i = 0; i < items.length; i++) {
          ...
          if (itemData is StylishMenuDividerData) {
            ...
            // 添加普通菜单项
            entries.add(
              PopupMenuItem<T>( // **标准 PopupMenuItem**
  ...
  Widget _buildItemContent(BuildContext context, Widget child, bool enabled, bool isSelected) {
    ...
    return Container(
      ...
      padding: itemPadding,
      child: DefaultTextStyle( // 统一设置文字样式
...
class StylishMenuDividerData<T> extends StylishMenuItemData<T?> {
  const StylishMenuDividerData() : super(value: null, child: const SizedBox.shrink());
}