...
import 'app_error_definition.dart'; // 导入干净的定义文件

/// 存储所有预定义的错误代码和信息
class AppErrorCodes {
  // --- Security Errors (SEC) --- (不可重试)
  static const rootDetected = AppErrorDefinition(
      code: "SEC-001",
      userMessageTemplate: "检测到设备已Root，为了您的账号安全，请使用未Root的设备",
      isRetryable: false);
  static const emulatorDetected = AppErrorDefinition(
      code: "SEC-002",
      userMessageTemplate: "不支持在模拟器中运行本应用",
      isRetryable: false);
  static const debugModeDetected = AppErrorDefinition(
      code: "SEC-003",
      userMessageTemplate: "请关闭开发者选项中的USB调试功能",
      isRetryable: false);
  static const multipleInstances = AppErrorDefinition(
      code: "SEC-004",
      userMessageTemplate: "应用已在运行中，请勿重复开启",
      isRetryable: false);
  static const unsafeEnvironment = AppErrorDefinition(
      code: "SEC-005",
      userMessageTemplate: "检测到不安全的运行环境，请关闭相关工具后重试",
      isRetryable: false);
  static const genericSecurityError = AppErrorDefinition(
      code: "SEC-100",
  ...
  static final Map<String, AppErrorDefinition> _definitionsByCode = {
  ...
  static AppErrorDefinition? getDefinitionByCode(String code) {
